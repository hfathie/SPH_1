...
def cooling_rate(T, nHI, nH2, n_tot):
	...
	#print(Lambda_CO, Lambda_H2, Lambda_HI, Lambda_HII)
...
def Lambda_from_Grid(ut, rhot):
	...
	n11 = np.where((uZ <= ut/unit_U) & (rhoZ <= rhot/unit_rho))[0]     # !!!!! Note the division by the u and rho Unit !
	
	return -LambdaZ[n11[-1]]
#-------------

#***** Lambda_from_GridX
def Lambda_from_GridZZZ(ut, rhot):

	n1 = np.where(uZ <= ut/unit_U)[0]
	LambdaZ1 = LambdaZ[n1]
	rhoZ1 = rhoZ[n1]
	
	n2 = np.where(rhoZ1 <= rhot/unit_rho)[0]
	
	return -LambdaZ1[n2[-1]]
#-------------



#===== DoCooling_M
def DoCooling_M_Lambda_from_Grid(rho, u_old, dt):
	...
	#TG = time.time()
	GammaLambdaNet = Lambda_from_Grid(u, rho) # coolingRateFromU_M(u, T_init, rho) # coolingRateFromU_M(u, nHcgs, XH) # !!!!! orrect ARGs
	#print('TG = ', time.time() - TG)
	...
	if (u - u_old - ratefact * GammaLambdaNet * dt > 0.0):
		...
		while (u_upper - u_old - ratefact * Lambda_from_Grid(u_upper, rho) * dt > 0.0):
			u_upper /= 1.1
	...
	while ((np.abs(du/u) > 1e-3) & (niter < MAXITER)):
	
		u = 0.5 * (u_lower + u_upper)
		
		T_init = (gamma - 1.0) * mH / kB * mu_approx * u # updating T_init with the last estimated u. Should make it faster.
		GammaLambdaNet = Lambda_from_Grid(u, rho)
...
#u_after_cooling = DoCooling_M_Lambda_from_Grid(rho_cgs, u_cgs, dt_cgs)

#print()
print('u before cooling = ', u)
print('u after cooling = ', u_after_cooling/unit_U)

print(u_to_temp(u_cgs, T_init, rho_cgs))

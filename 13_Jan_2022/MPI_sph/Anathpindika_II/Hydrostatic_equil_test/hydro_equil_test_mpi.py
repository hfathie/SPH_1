...
def P_polytrop_mpi(nbeg, nend, rho, T_cld, T_ps, T_0):
	...
	for i in range(nbeg, nend):
		...
		elif (rhot >= 1.e-21) and (rhot <= 2.e-21):
			P_res[i-nbeg] = rhot * const * gamma * T_cld * (rhot/2.e-21)**(gamma-1.)
		
		elif (rhot >= 2.e-21) and (rhot <= 1.e-18):
			P_res[i-nbeg] = rhot * const * T_ps
		
		elif rhot > 1.e-18:
			P_res[i-nbeg] = rhot * const * T_0 * (1. + gamma * (rhot/1.e-14)**(gamma-1.))
...
def sound_speed_mpi(nbeg, nend, rho, T_cld, T_ps, T_0):
	...
	for i in range(nbeg, nend):
		...
		if (rhot >= 2.e-21) and (rhot <= 1.e-18):
...
while t < tEnd:
	...
	if rank == 0:
		ii += 1
		dictx = {'pos': r, 'v': v, 'm': m, 'u': 0., 'dt': dt, 'current_t': t, 'rho': rho, 'h': h}

...
#----- P_polytrop_mpi
@njit
def P_polytrop_mpi(nbeg, nend, rho, c_iso, K, gamma):
	...
	for i in range(nbeg, nend):
		...
		P_res[i-nbeg] = rhot * c_iso * c_iso + K * rhot**gamma
...
UnitRadius_in_cm = 4.99e16  #!!!!!!!!!!!!!! CHANGE !!!!!!!!!!!!!!!!!!
...
if rank == 0:
	filz = np.sort(os.listdir(dirx))
	
	print(filz[-1])

	with open(dirx + filz[-1], 'rb') as f:
		data = pickle.load(f)

	ii = int(filz[-1].split('.')[0]) + 1
...
print('unitTime_in_kyr = ', unitTime/3600./24./365.25/1000.)
...
rho_crit = 5.0e-14 # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
K = c_iso*c_iso * rho_crit ** (1.-gamma)
...
#--------- p ----------
P = 0.
local_P = P_polytrop_mpi(nbeg, nend, rho, c_iso, K, gamma)

...
@njit
def getAcc_sph_shear_mpiZZZZ(nbeg, nend, pos, v, rho, P, c, h, m, divV, curlV, alpha): # shear viscosity-corrected.
...
def getAcc_sph_shear_mpi(nbeg, nend, pos, v, rho, P, c, h, m, divV, curlV, alpha, beta, eta): # shear viscosity-corrected.
	...
	for i in range(nbeg, nend):
		...
		delta_t_cv[i-nbeg] = h[i] / (c[i] + 0.6 * (alpha*c[i] + beta*np.max(mu_i)))
...
@njit
def get_dU_shear_mpiZZZZ(nbeg, nend, pos, v, rho, P, c, h, m, divV, curlV, alpha):

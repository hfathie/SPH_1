...
plt.scatter(res[:, 1], res[:, 2], s = 1, color = 'k')
plt.show()
...
dictx = {'r': res, 'v':np.zeros_like(res), 'h': h}

with open('Data_Turner_Random.pkl', 'wb') as f:
	pickle.dump(dictx, f)
#----------------------------

plt.figure().add_subplot(111, projection='3d').scatter(res[:, 0], res[:, 1], res[:, 2], s = .1);
plt.show()
